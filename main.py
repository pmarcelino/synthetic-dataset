...
def evaluate_questions(row) -> pd.DataFrame:
    text = row["text"]
...
def evaluate_answers(row) -> pd.DataFrame:
    text = row["text"]
    question = row["question"]
    answer = row["answer"]
    llm_evaluation_question = row["llm_evaluation_question"]
...
run_questions_and_answers_generation = False
...
# Evaluate questions
run_questions_evaluation = False
if run_questions_evaluation:
    df = pd.read_csv("dataset.csv")

    with ThreadPoolExecutor(num_threads) as executor:
        results = list(executor.map(evaluate_questions, df.to_dict(orient="records")))

    for index, result in enumerate(results):
        df.at[index, "llm_evaluation_question"] = result
...
with ThreadPoolExecutor(num_threads) as executor:
    results = list(executor.map(evaluate_answers, df.to_dict(orient="records")))

for index, result in enumerate(results):
    df.at[index, "llm_evaluation_answer"] = result
